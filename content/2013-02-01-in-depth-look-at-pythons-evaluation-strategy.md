...
printed, like when calling `print()` on strings and integers. More often, though, 
the interpreter spits out some odd looking string like:
...
address in memory where the object `foo` is bound to is stored. This isn't as 
...
In face, we can see all the attributes `10` has using the `dir()` function:
...
the real `datetime.datetime` class. Except for the fact that if you call
`datetime.datetime.now()` it always prints out 'Party Time!', of course.

Obviously this is a silly example, but hopefully it gives you some insight into
what is possible when you fully understand Python's execution model. At this point, 
though, we've only changed bindings associated with a name. What about
...
wasn't the whole truth. A number of containers in Python, like `tuple` are 
...
unchangeable bindings to an object. The key thing to note is that 
...
unchanged. As you'll recall, assinging `root` to `root.setdefault(character, {})` 
merely rebinds `root` to the object created by 
the `root.setdefault(character, {})` statement.
    ...
    def print_some_weird_calculation(value):
        ...
        def print_formatted_calculation(result):
            print('{value} * {constant} = {result}'.format(value=value,
                constant=GLOBAL_CONSTANT, result=reslut))
...
blocks are modules, class definitions, and the body of a function. So the
...
the body of the function. It's `scope` would not include
`print_formatted_calculation`, since that `block` rebound the name.

Also, there are two keywords that can be used to tell the interpreter to
*use a preexisting binding*. Every other time we bind a name, it binds that name
to a new object, *but only for the current scope*. Like in the example above, if
we rebound `value` in `print_formatted_calculation`, it would have no affect on
the `value` in `print_some_weird_calculation`, its enclosing scope. With the
...
name `my_variable` in the top-most scope (and if it doesn't find it there, create 
the name in the `global` scope). Putting `global my_variable` in a code block 
is a way of saying "copy the binding of this global variable, or if it already 
exists, I'll use that binding." Similarly, the `nonlocal` statement instructs 
the interpreter to search use the binding of a name in the nearest *enclosing* 
scope. This is a way to *rebind* a name not defined in either the 
local or global scope. Without `nonlocal`, we would either be creating new
bindings in the local scope or using a binding from the global scope.
Unlike `global` however, the name being searched for must already exist; it 
won't be created if it's not found.

To see this in action, let's see a quick example:
...
By making use of `global` and `nonlocal`, we're able to use and change existing
bindings of a name rather than merely assigning the name a new binding and
losing the old one.
